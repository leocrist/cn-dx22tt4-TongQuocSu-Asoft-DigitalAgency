...
export async function GET(request: NextRequest, { params }: { params: { slug: string } }) {
  ...
  try {
    // Get the work
    const { data: work, error } = await supabase.from("works").select("*, work_details(*)").eq("slug", slug).single()
...
export async function PUT(request: NextRequest, { params }: { params: { slug: string } }) {
  try {
    ...
    if (body.details && body.details.length > 0) {
      ...
      const { error: deleteError } = await supabase.from("work_details").delete().eq("work_id", existingWork.id)
      ...
      const { error: insertError } = await supabase.from("work_details").insert(detailsToInsert)
    ...
    return NextResponse.json({ success: false, message: "Failed to update work" }, { status: 400 })
  }
}
